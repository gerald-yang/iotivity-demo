...
std::shared_ptr<OCResource> sensorResourceA;
std::shared_ptr<OCResource> ledResource;
std::shared_ptr<OCResource> ledResourceA;
...
void onPutLedA(const HeaderOptions& /*headerOptions*/, const OCRepresentation& rep, const int eCode)
{
	try {
		if(eCode == OC_STACK_OK) {
			...
			rep.getValue("status", mydemo.led_status_a);

			std::cout << "\tArduino LED status: " << mydemo.led_status_a << std::endl;
		} else {
			std::cout << "onPutLed Response error: " << eCode << std::endl;
			std::exit(-1);
		}
...
void onPutLcdA(const HeaderOptions& /*headerOptions*/, const OCRepresentation& rep, const int eCode)
{
	try {
		if(eCode == OC_STACK_OK) {
			...
			rep.getValue("lcd", mydemo.lcd_str_a);

			std::cout << "\tLCD string: " << mydemo.lcd_str_a << std::endl;
		} else {
...
void putLedRepresentationA(std::shared_ptr<OCResource> resource)
{
	if(resource) {
		...
		rep.setValue("status", mydemo.led_status_a);
...
void putLcdRepresentationA(std::shared_ptr<OCResource> resource)
{
	if(resource) {
		...
		rep.setValue("lcd", mydemo.lcd_str_a);
...
void onGetSensorA(const HeaderOptions& /*headerOptions*/, const OCRepresentation& rep, const int eCode)
{
	try {
		if(eCode == OC_STACK_OK) {
			...
			rep.getValue("temperature", mydemo.sensor_temp_a);
			rep.getValue("light", mydemo.sensor_light_a);
			rep.getValue("sound", mydemo.sensor_sound_a);

			std::cout << "\ttemperature: " << mydemo.sensor_temp_a << std::endl;
			std::cout << "\tlight: " << mydemo.sensor_light_a << std::endl;
			std::cout << "\tsound: " << mydemo.sensor_sound_a << std::endl;
...
void onGetLedA(const HeaderOptions& /*headerOptions*/, const OCRepresentation& rep, const int eCode)
{
	try {
		if(eCode == OC_STACK_OK) {
			...
			std::cout << "\tLED status: " << mydemo.led_status_a << std::endl;
		} else {
			std::cout << "onGET Response error: " << eCode << std::endl;
			std::exit(-1);
		}
...
static void led_write_a(int status)
{
	mydemo.led_status_a = status;
...
static void lcd_write_a(std::string str)
{
	mydemo.lcd_str_a = str;
